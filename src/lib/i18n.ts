...
const TRANSIT_REASON_ZH: Record<string, string> = {
  "Do BC - Nhận hàng trễ": "网点原因 - 收件延误",
  "Do BC - Gửi hàng về trễ": "网点原因 - 发件延误",
  "Do HUB - Thao tác trễ": "HUB原因 - 操作延误",
  "Do HUB - Do xe trục trặc": "HUB原因 - 车辆故障",
  "Khác": "其他",
};
...
const vi = {
  ...
  pages: {
    ...
    receivingLevel: {
      ...
      statTotalLabel: "Tổng đơn thành công",
      ...
      khuBoxSubtitle: "Khu ≥ 1.000 đơn, sắp xếp theo tỷ lệ thành công tăng dần",
      bcBoxSubtitle: "Bưu cục ≥ 300 đơn, sắp xếp theo tỷ lệ thành công tăng dần",
      khuEmptyMsg: "Không có khu nào đạt ngưỡng ≥ 1.000 đơn trong khoảng thời gian này.",
      bcEmptyMsg: "Không có bưu cục nào đạt ngưỡng ≥ 300 đơn trong khoảng thời gian này.",
    },
    transit: {
      pageTitle: "Trung chuyển — HUB",
      subtitle: 'Nguồn: thư mục "Trung chuyển - HUB" · chỉ tính đơn có trung chuyển qua HUB thật',
      statTongDon: "Tổng đơn qua HUB",
      statDungGio: "Đúng giờ",
      statTre: "Trễ",
      statCountPct: (count: string, pct: number) => `${count} (${pct}%)`,
      trendChartTitle: "Tỷ lệ đúng giờ theo ngày (%)",
      hubChartTitle: "Xếp hạng theo TTTC (HUB) — tỷ lệ đúng giờ %",
      hubChartNote: "Sắp xếp tăng dần — HUB ở đầu danh sách cần chú ý nhất",
      hubTableTitle: "Bảng chi tiết theo TTTC (HUB)",
      reasonChartTitle: "Nguyên nhân trễ (số đơn)",
      rootCauseNote:
    ...
    hubCoverage: {
      ...
      filterKhu: "Khu",
      filterQuan: "Quận (cũ)",
      filterPhuong: "Phường/Xã",
      filterAll: "Tất cả",
      filterUnknown: "Không xác định",
      filterReset: "Xoá bộ lọc",
      filterQuanUnavailable: "Dữ liệu Khâu nhận không có mã phường nên không suy ra được Quận cũ — bộ lọc Quận chỉ dùng được ở chế độ Hàng phát.",
    },
    weeklyMeeting: {
      ...
      findingTransitWorstHub: (hub: string, pct: number, total: string) =>
        `Trung chuyển: TTTC ${hub} có tỷ lệ đúng giờ thấp nhất (${pct}%, trên ${total} đơn).`,
      ...
      recoTransitHub: (hub: string, pct: number) =>
        `Rà soát quy trình vận hành tại TTTC ${hub} — tỷ lệ đúng giờ chỉ ${pct}%, thấp hơn đáng kể so với mặt bằng chung.`,
...
const zh: typeof vi = {
  ...
  pages: {
    ...
    receivingLevel: {
      ...
      statTotalLabel: "成功揽收总单量",
      statAvgLabel: "日均成功揽收单量",
      statRateLabel: "揽收成功率",
      chartTitle: "每日成功揽收单量与揽收成功率",
      chartEmptyMsg: "请在上方筛选中选择1个片区或网点以查看图表。",
      detailTitle: "分级明细",
      khuBoxSubtitle: "片区单量 ≥1,000，按揽收成功率升序排列",
      bcBoxSubtitle: "网点单量 ≥300，按揽收成功率升序排列",
      khuEmptyMsg: "所选时间范围内没有片区单量达到 ≥1,000 的门槛。",
      bcEmptyMsg: "所选时间范围内没有网点单量达到 ≥300 的门槛。",
    },
    transit: {
      pageTitle: "转运 — HUB",
      subtitle: '数据来源："Trung chuyển - HUB" 文件夹 · 仅统计经过真实HUB转运的订单',
      statTongDon: "经HUB总单量",
      statDungGio: "准时",
      statTre: "延误",
      statCountPct: (count: string, pct: number) => `${count}（${pct}%）`,
      trendChartTitle: "每日准时率 (%)",
      hubChartTitle: "各转运中心（HUB）排名 — 准时率 %",
    ...
    hubCoverage: {
      ...
      filterUnknown: "未识别",
      filterReset: "清除筛选",
      filterQuanUnavailable: "揽收数据不含坊级编码，无法推导原辖区 — 辖区筛选仅在派送模式下可用。",
    },
    weeklyMeeting: {
      pageTitle: "周会报告",
      ...
      findingTransitWorstHub: (hub: string, pct: number, total: string) => `转运：转运中心 ${hub} 准时率最低（${pct}%，共 ${total} 单）。`,
      ...
      recoTransitHub: (hub: string, pct: number) => `建议排查转运中心 ${hub} 的运作流程 — 准时率仅 ${pct}%，明显低于整体水平。`,

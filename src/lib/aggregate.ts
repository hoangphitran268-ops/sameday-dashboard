...
function aggregateNodeDays(days: RawNodeDay[], minVolume: number, labelKey: "buu_cuc" | "khu") {
  const byKey = new Map<string, { tong_don: number; da_ky_nhan: number; issue: number; dur_sum: number; dur_count: number }>();
  for (const d of days) {
    const acc = byKey.get(d.key) ?? { tong_don: 0, da_ky_nhan: 0, issue: 0, dur_sum: 0, dur_count: 0 };
...
export function aggregateRange(raw: RawDashboardData, range: DateRange): DashboardData {
  ...
  // buu cuc / khu perf — xếp hạng theo tỷ lệ ký nhận (đơn được xử lý xong trong ngày N),
  // đây là chỉ số biến động thực sự; "cùng ngày" đã bỏ vì luôn ~100% khi đã ký nhận (trùng lặp).
  ...
  const bc_worst15 = [...bcAll].sort((a, b) => a.ty_le_ky_nhan_pct - b.ty_le_ky_nhan_pct).slice(0, 15);
  const bc_best15 = [...bcAll].sort((a, b) => b.ty_le_ky_nhan_pct - a.ty_le_ky_nhan_pct).slice(0, 15);
  const khu_perf = aggregateNodeDays(khuDays, 1000, "khu").sort((a, b) => a.ty_le_ky_nhan_pct - b.ty_le_ky_nhan_pct);
